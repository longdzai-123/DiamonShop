				...
				<table class="table table-bordered table-condensed">
					...
					<tbody>
						 <c:forEach var="item" items="${Cart}">
						      <tr>
								<td><img width="100" src="<c:url value="/assets/user/img/${item.value.productsDto.img}"/>" alt=""></td>
								<td>${item.value.productsDto.title}</td>
								<td><span class="shopBtn" style="background-color: ${item.value.productsDto.code_color};"><span class="icon-ok"></span></span></td>
								<td><fmt:formatNumber type="number" groupingUsed="true" value="${item.value.productsDto.price}" /> ₫  </td>
								<td><input type="number" min ="0" max ="1000" class="span1" style="max-width: 34px" placeholder="1" id="quanty-cart-${item.key}" size="16"  value="${item.value.quanty}"></td>
								<td>
